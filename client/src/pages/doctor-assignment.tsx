...
export default function DoctorAssignment() {
  ...
  const assignedPatients = allPatients.filter(
    (patient: Patient) => patient.status === "in_treatment",
  );
      ...
      <div className="container mx-auto px-6 py-8 max-w-7xl">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-muted-foreground">
                Assign patients to appropriate doctors based on their medical
                conditions
              </p>
            </div>
            <Link href="/">
        ...
        <div className="mb-8 grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {doctors.map((doctor) => {
            ...
            return (
              <Card
                key={doctor.id}
                className={`medical-card ${isBusy ? "opacity-60" : ""}`}
                data-testid={`doctor-card-${doctor.id}`}
              >
                <CardContent className="p-4 text-center">
                  <div
                    className={`w-12 h-12 ${isBusy ? "bg-red-100" : "bg-primary/10"} rounded-full flex items-center justify-center mx-auto mb-2`}
                  >
                    <span
                      className={`${isBusy ? "text-red-600" : "text-primary"} font-semibold text-sm`}
                    >
                      {doctor.avatar}
                    </span>
                  </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-muted-foreground mt-1">
                  Assign patients to doctors and manage treatment flow
                </p>
              </div>
          ...
          <CardContent className="p-6">
            {!queueData || queueData.length === 0 ? (
              <div
                ...
                data-testid="empty-assignment-queue"
              >
                ...
                <p className="text-muted-foreground">
                  All patients have been assigned to doctors or the queue is
                  empty.
                </p>
              </div>
            ) : (
              <div className="space-y-6" data-testid="assignment-list">
                {queueData.map((patient: Patient) => {
                  const priority =
                    priorityConfig[
                      patient.triageLevel as keyof typeof priorityConfig
                    ];
                    ...
                    >
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex items-center space-x-4">
                          <div
                            className={`w-10 h-10 ${priority.color} rounded-full flex items-center justify-center font-bold`}
                          >
                            {patient.triageLevel}
                          </div>
                          <div>
                            ...
                            <p className="text-sm text-muted-foreground">
                              Age: {patient.age} • Waiting:{" "}
                              {getTimeAgo(patient.timestamp)}
                            </p>
                            <Badge
                              className={priority.color}
                              data-testid={`priority-badge-${patient.id}`}
                            >
                              {priority.label}
                            </Badge>
                          </div>
                      ...
                      <div className="bg-muted/50 rounded-lg p-3">
                        ...
                        <p
                          ...
                          data-testid={`diagnosis-${patient.id}`}
                        >
                          {patient.diagnosis}
                        </p>
                      </div>
                      ...
                      <div className="flex items-center justify-between space-x-4">
                        <div className="flex-1">
                          ...
                          <Select
                            ...
                            onValueChange={(value) =>
                              handleDoctorSelect(patient.id, value)
                            }
                          ...
                          >
                            ...
                            <SelectContent>
                              {recommendedDoctors &&
                                Array.isArray(recommendedDoctors) &&
                              ...
                              {Array.isArray(getAvailableDoctors()) &&
                        ...
                        <Button
                          ...
                          disabled={
                            !selectedDoctor || assignPatientMutation.isPending
                          }
        ...
        <Card className="shadow-lg medical-card">
          <CardHeader className="border-b border-border">
            ...
            <p className="text-muted-foreground">
              Manage patients currently being treated by doctors
            </p>
          </CardHeader>
          <CardContent className="p-6">
            {assignedPatients.length === 0 ? (
              <div className="text-center py-12 space-y-2">
                ...
                <p className="text-muted-foreground">
                  All patients are either waiting for assignment or have
                  completed treatment.
                </p>
              </div>
            ) : (
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-4">
                          ...
                          <div>
                            <h3
                              ...
                              data-testid={`treatment-patient-name-${patient.id}`}
                            >
                              {patient.name}
                            </h3>
                            <p className="text-sm text-muted-foreground">
                              Age: {patient.age} • Assigned to:{" "}
                              {assignedDoctor?.name} • Treatment started:{" "}
                              {getTimeAgo(patient.timestamp)}
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center space-x-4">
                          <div className="text-right">
                            ...
                            <p
                              className="text-sm"
                              data-testid={`treatment-diagnosis-${patient.id}`}
                            >
                              {patient.diagnosis}
                            </p>
                          </div>
                          ...
                          <Button
                            onClick={() =>
                              completePatientMutation.mutate(patient.id)
                            }
                          ...
                          >
                            ...
                            {completePatientMutation.isPending
                              ? "Completing..."
                              : "Complete Treatment"}
                          </Button>
                        </div>
